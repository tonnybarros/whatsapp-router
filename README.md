...
- suporte a conectores `custom` independentes por workspace.
...
Use `provider: "custom"` quando o cliente/workspace precisar conectar uma API propria, diferente de `uazapi`, `waha`, `evolution_go` ou `evolution_api`. O Router continua cuidando de selecao do conector, limite diario, intervalo minimo, fila, cooldown, failover e historico por workspace; a diferenca e que voce informa como montar a chamada HTTP para a API externa.
...
Exemplo de conector customizado em um workspace:

```json
{
  "name": "api-custom-cliente",
  "provider": "custom",
  ...
  "custom_headers": "{\"X-Workspace\":\"cliente-a\"}",
...
Com esse template, um envio para `5511999999999` com mensagem `Ola` gera um POST para `https://api.cliente.com/send` com corpo parecido com:
